# === USAGE ===
# Run from Blender Scripting tab or via terminal:
# blender camera_in_tube.blend --background --python raytrace_script.py
#
# Description:
# Casts rays from camera frustum, applies Snell’s Law at tube interfaces,
# and detects intersection on a plane. Outputs 4-column CSV or visualizes rays.
# ========================
...
# === USER PARAMETERS ===
MODE = 'display'  # 'display' or 'export'
GRID_WIDTH = 32
GRID_HEIGHT = 20
HFOV_DEG = 127.0
VFOV_DEG = 79.5
...
def refract_ray(incident, normal, n1, n2):
    ...
    if incident.dot(normal) > 0:
        normal = -normal  # flip if hitting backface
...
def get_camera_ray(cam_obj, u, v):
    ...
    pixel_cam_x = (ndc_x - 0.5) * sensor_width
    pixel_cam_y = (0.5 - ndc_y) * sensor_height
    pixel_cam_z = -1.0  # rays shoot forward along negative Y

    local_dir = mathutils.Vector((pixel_cam_z, pixel_cam_y, pixel_cam_x)).normalized()
...
def main():
    cam = bpy.data.objects['Camera_001']
    tube = bpy.data.objects['tube']
    plane = bpy.data.objects['hit_plane']
    ...
    for i in range(GRID_HEIGHT):
        for j in range(GRID_WIDTH):
            ...
            results.append((j, i, final_hit.x, final_hit.y))

    print(f"Ray tracing complete. Total samples: {len(results)}")
    print(f"Valid final hits: {sum(1 for r in results if r[2] != '')}")
